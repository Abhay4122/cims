$(document).ready(function (a) {
  ...
  a('.skill-shortcode').appear(
    ...
    a('#email_submit').click(function () {
      ...
      if (((b = /^[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$/), '' == d || 'Email' == d || !b.test(d)))
        return a('input#email').val(''), a('input#email').focus(), !1
      ...
      return (
        a('#reply_message').addClass('email_loading'),
        a.ajax({
          type: 'POST',
          url: '../php/sendEmail.php',
          data: f,
          success: function () {
            a('#reply_message').removeClass('email_loading'),
              a('#reply_message').addClass('list3'),
              a('#reply_message').html('Well done, You are successful !').hide().fadeIn(1500)
          }
        }),
        !1
      )
    ...
    a("a[data-gal^='prettyPhoto']").prettyPhoto({ hook: 'data-gal' }),
    a(window).scroll(function () {
    ...
    a('.page-scroll a').bind('click', function (b) {
      ...
      a('html, body')
        .stop()
        .animate({ scrollTop: a(c.attr('href')).offset().top }, 1500, 'easeInOutExpo'),
        b.preventDefault()
    }),
    a('#intro-slider').flexslider({ animation: 'fade', controlNav: !1, DirectionNav: !1, slideshowSpeed: 4e3, animationSpeed: 600 }),
    a('.dropdown').hover(
      function () {
